...
//        }).start();
    ...
    private String cameraInfo() {
        ...
        synchronized (fishes) {
            ...
            List<Fish> livingFishes = new ArrayList<>(fishes.stream().parallel().filter(fish -> fish.getLifeSpan() != 0).toList());

            if (maleFishCount == livingFishes.size() || maleFishCount == 0)
            ...
            Collections.sort(livingFishes);
            checkCoordinate(livingFishes, stringBuilder);

            return stringBuilder.isEmpty() ? stringBuilder.append("Baliqlar ko'paymadi.\n").toString() : stringBuilder.toString();
        }
    }

    private void checkCoordinate(List<Fish> livingFishes, StringBuilder stringBuilder) {
        Fish fish1;
        Fish fish2;
        for (int i = 0; i < livingFishes.size() - 1; i++) {
            for (int j = i + 1; j < livingFishes.size(); j++) {
                fish1 = livingFishes.get(i);
                fish2 = livingFishes.get(j);
...
//                new Thread(newFish).start();
                Thread.ofVirtual().start(newFish);

                stringBuilder.append(
                        String.format(
                                "%s - id'lik %s bilan %s - id'lik %s uchrashdi \n va %s - id'lik %s tug'ildi.\n\n",
